...
class UriJavaUriSpec extends FreeSpec with GeneratorDrivenPropertyChecks with ShouldMatchers with TryValues {

  implicit override val generatorDrivenConfig = PropertyCheckConfig(maxDiscarded = 2000)
...
//    assert(u.authority.isEmpty)
    ...
    assert(Uri.parse(su).success.value === u, s"""su = "$su" """)
  }
